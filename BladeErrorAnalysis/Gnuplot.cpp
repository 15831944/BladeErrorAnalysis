...
std::string Gnuplot::m_sGNUPlotPath = "C:/Workspace/VisualStudio/VS2013/BladeErrorAnalysis/BladeErrorAnalysis/gnuplot";
...
{
	std::string temp = path + "/" + m_sGNUPlotFileName;
	if (Gnuplot::file_exists(temp, 0)) // check existence
	{
...
std::string Gnuplot::create_tmpfile(std::ofstream &tmp)
{
	...
	//
	int sizeInChars = strnlen(name, 16) + 1;
	int err = _mktemp_s(name, sizeInChars);
	if (err != NULL)
	{
		std::ostringstream except;
...
// Plots a linear equation y=ax+b (where you supply the slope a and intercept b)
...
Gnuplot& Gnuplot::plotfile_xy(const std::string &filename, const unsigned int column_x, const unsigned int column_y, const std::string &title)
{
	// check if file exists
	file_available(filename);
	std::ostringstream cmdstr;
...
Gnuplot& Gnuplot::plotfile_xy_err(const std::string &filename, const unsigned int column_x, const unsigned int column_y, const unsigned int column_dy, const std::string &title)
{
	// check if file exists
	file_available(filename);
	std::ostringstream cmdstr;
...
Gnuplot& Gnuplot::plotfile_xyz(const std::string &filename, const unsigned int column_x, const unsigned int column_y, const unsigned int column_z, const std::string &title)
{
	// check if file exists
	file_available(filename);
	std::ostringstream cmdstr;
	...
	if (title == "")
		...
		cmdstr << " title \"" << title << "\" with " << pstyle << " pointtype 7 pointsize 2";
	//
	// Do the actual plot
	//
	cmd(cmdstr.str());
...
Gnuplot& Gnuplot::plot_image(const unsigned char * ucPicBuf, const unsigned int iWidth, const unsigned int iHeight, const std::string &title)
{
	std::ofstream tmp;
